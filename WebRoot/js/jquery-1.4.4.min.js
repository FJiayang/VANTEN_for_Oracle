...
(function(E, B) {
	...
	function ma(a, b, d) {
		if (c.isFunction(b)) return c.grep(a, function(f, h) {
			...
			if (Na.test(b)) return c.filter(b, e, !d);else
				b = c.filter(b, e)
		}
				...
				init : function(j,
					...
					if (typeof j === "string")
						if ((v = h.exec(j)) && (v[1] || !s))
							if (v[1]) {
								...
								if (z = A.exec(j))
									if (b.isPlainObject(s)) {
										j = [ t.createElement(z[1]) ];b.fn.attr.call(j, s, true)
									} else
										j = [ H.createElement(z[1]) ];else {
							...
							this.selector = j;
							this.context = t;
							j = t.getElementsByTagName(j);return b.merge(this, j)
			...
			b.extend = b.fn.extend = function() {
				...
				bindReady : function() {
					if (!p) {
						...
						if (t.addEventListener) {
							t.addEventListener("DOMContentLoaded", u, false);E.addEventListener("load", b.ready, false)
							...
							} catch (s) {} t.documentElement.doScroll && j && a()
						}
					}
				},
				...
				globalEval : function(j) {
					if (j &&
						...
						if (b.support.scriptEval) v.appendChild(t.createTextNode(j));else
							v.text = j;
			...
			if (t.addEventListener)
				...
				u = function() {
					if (t.readyState === "complete") {
						t.detachEvent("onreadystatechange", u);b.ready()
					}
				};
			return E.jQuery = E.$ = b
		}();
	(function() {
		...
		if (!(!f || !f.length || !h)) {
			...
			} catch (o) {} a.insertBefore(b, a.firstChild);
			if (E[e]) {
			...
			a = t.createDocumentFragment();a.appendChild(d.firstChild);
			c.support.checkClone = a.cloneNode(true).cloneNode(true).lastChild.checked;c(function() {
				var r = t.createElement("div");
				r.style.width = r.style.paddingLeft = "1px";t.body.appendChild(r);
	...
	c.extend({
		...
		data : function(a, b, d) {
			if (c.acceptData(a)) {
				...
				if (!(e && !f && typeof b === "string" && d === B)) {
					if (e) f || (a[c.expando] = f = ++c.uuid);else
						h = a;
					if (typeof b === "object")
						if (e)
							h[f] = c.extend(h[f], b);else c.extend(h, b);
					else if (e && !h[f])
						h[f] = {};
					a = e ? h[f] : h;
					if (d !== B)
		...
		data : function(a, b) {
			...
			if (b === B) {
				...
				return d === B && k[1] ? this.data(k[0]) : d
					...
					var o = c(this),
						x = [ k[0], b ];
					o.triggerHandler("setData" + k[1] + "!", x);c.data(this, a, b);o.triggerHandler("changeData" + k[1] + "!", x)
				})
		},
		...
		queue : function(a, b, d) {
			if (a) {
				...
				if (!d) return e || [];
				if (!e || c.isArray(d))
					e = c.data(a, b, c.makeArray(d));else e.push(d);
				return e
			}
		},
		dequeue : function(a, b) {
			...
			if (e === "inprogress")
				e = d.shift();
			if (e) {
				b === "fx" && d.unshift("inprogress");e.call(a, function() {
					c.dequeue(a, b)
				})
			}
		}
			...
			return this.each(function() {
				if (d === "string")
					for (var f, h = 0, l = c(this), k = b, o = a.split(ha); f = o[h++];) {
						k = e ? k : !l.hasClass(f);l[k ? "addClass" : "removeClass"](f)
				}
	...
	c.event = {
					...
					for (var x = 0, r; l = b[x++];) {
						...
						if (!A) {
							...
							if (!C.setup || C.setup.call(a, e, r, o) === false)
								if (a.addEventListener) a.addEventListener(l, o, false);else a.attachEvent && a.attachEvent("on" + l, o)
						}
						if (C.add) {
							C.add.call(a, h);
							if (!h.handler.guid)
									...
									for (h = e || 0; h < A.length; h++) {
										...
										if (d.guid === C.guid) {
											if (k || x.test(C.namespace)) {
												e == null && A.splice(h--, 1);r.remove && r.remove.call(a, C)
											}
											if (e != null) break
										}
									}
		...
		trigger : function(a, b, d, e) {
			...
			e = d.parentNode || d.ownerDocument;try {
			...
			Z = function(a, b) {
				...
				if (!(!ia.test(d.nodeName) || d.readOnly)) {
					...
					if (!(e === B || f === e))
						if (e != null || f) {
							a.type = "change";
							a.liveFired = B;return c.event.trigger(a, b, d)
					}
				}
			};
		c.event.special.change = {
			filters : {
				...
				beforeactivate : function(a) {
					a = a.target;c.data(a, "_change_data", xa(a))
				}
			},
			setup : function() {
				if (this.type ===
			...
			teardown : function() {
				c.event.remove(this, ".specialChange");return ia.test(this.nodeName)
			}
		};
	...
	t.addEventListener && c.each({
		...
		c.fn[b] = function(d, e, f) {
			...
			var l = b === "one" ? c.proxy(f, function(o) {
				c(this).unbind(o, l);return f.apply(this, arguments)
			}) : f;
	...
	(function() {
		...
		k.filter = function(g, i, n, m) {
			for (var p, q, u = g, y = [], F = i, M = i && i[0] && k.isXML(i[0]); g && i.length;) {
				...
				if (g === u)
					if (q == null) k.error(g);else break;
				u = g
			}
			return F
		};
					...
					PSEUDO : function(g, i, n, m, p) {
						if (g[1] === "not")
							if ((d.exec(g[3]) || "").length > 1 || /^\w/.test(g[3]))
								g[3] = k(g[3], null, null, i);else {
								g = k.filter(g[3], i, n, true ^ p);n || m.push.apply(m, g);return false
						}
		...
		k.getText = function(g) {
			...
			var g = t.createElement("div"),
				...
				n = t.documentElement;
			g.innerHTML = "<a name='" + i + "'/>";n.insertBefore(g, n.firstChild);
			...
			if (i)
				k.matchesSelector = function(p, q) {
					...
					if (!k.isXML(p)) try {
						...
						} catch (u) {} return k(q, null, null, [ p ]).length > 0
			}
		})();(function() {
			var g = t.createElement("div");
			...
			if (c.isArray(a)) {
				...
				if (h && a.length) {
					...
					for (f = a.length; e < f; e++) {
						l = a[e];k[l] || (k[l] = c.expr.match.POS.test(l) ? c(l, b || this.context) : l)
					}
	...
	c.fn.extend({
		...
		clone : function(a) {
			var b = this.map(function() {
				if (!c.support.noCloneEvent && !c.isXMLDoc(this)) {
					...
					if (!d) {
						d = e.createElement("div");d.appendChild(this.cloneNode(true));
						d = d.innerHTML
					}
		...
		domManip : function(a, b, d) {
			...
			if (c.isFunction(l)) return this.each(function(x) {
					var r = c(this);
					a[0] = l.call(this, x, b ? r.html() : B);r.domManip(a, b, d)
				});
			if (this[0]) {
				e = l && l.parentNode;
	...
	c.buildFragment = function(a, b, d) {
		...
		if (!e) {
			e = b.createDocumentFragment();c.clean(a, b, e, d)
		}
		if (f)
	...
	c.curCSS = c.css;c.each([ "height", "width" ], function(a, b) {
		c.cssHooks[b] = {
			get : function(d, e, f) {
				...
				if (e) {
					if (d.offsetWidth !== 0)
						h = oa(d, b, f);else c.swap(d, kb, function() {
							h = oa(d, b, f)
						});
					if (h <= 0) {
						h = W(d, b, b);
					...
					if (h < 0 || h == null) {
						h = d.style[b];return h === "" || h === "auto" ? "0px" : h
					}
	...
	c.fn.extend({
			...
			if (w) {
				b.username ? w.open(h, b.url, b.async, b.username, b.password) : w.open(h, b.url, b.async);try {
					if (b.data != null && !l || a && a.contentType) w.setRequestHeader("Content-Type",
							b.contentType);
					if (b.ifModified) {
						c.lastModified[b.url] && w.setRequestHeader("If-Modified-Since", c.lastModified[b.url]);c.etag[b.url] && w.setRequestHeader("If-None-Match", c.etag[b.url])
					}
				...
				if (b.beforeSend && b.beforeSend.call(b.context, w, b) === false) {
					b.global && c.active-- === 1 && c.event.trigger("ajaxStop");w.abort();return false
				}
				b.global &&
				...
				var L = w.onreadystatechange = function(m) {
					if (!w || w.readyState === 0 || m === "abort") {
						...
						d || c.handleComplete(b, w, e, f);m === "timeout" && w.abort();
						if (b.async)
							w = null
					}
				};
				try {
					var g = w.abort;
					w.abort = function() {
						w && Function.prototype.call.call(g, w);L("abort")
					}
				} catch (i) {} b.async && b.timeout > 0 && setTimeout(function() {
					w && !J && L("timeout")
				}, b.timeout);try {
					w.send(l || b.data == null ? null : b.data)
				} catch (n) {
					c.handleError(b, w, null, n);c.handleComplete(b, w, e, f)
				} b.async || L();return w
			}
		},
		param : function(a, b) {
		...
		handleError : function(a, b, d, e) {
			a.error && a.error.call(a.context, b, d, e);a.global && c.triggerGlobal(a, "ajaxError", [ b, a, e ])
		},
		...
		handleComplete : function(a, b, d) {
			a.complete && a.complete.call(a.context, b, d);a.global && c.triggerGlobal(a, "ajaxComplete", [ b, a ]);a.global && c.active-- === 1 && c.event.trigger("ajaxStop")
		},
	...
	c.fn.extend({
		...
		animate : function(a, b, d, e) {
			...
			return this[f.queue === false ? "each" : "queue"](function() {
				...
				h.curAnim = c.extend({}, a);c.each(a, function(A, C) {
		...
		speed : function(a, b, d) {
			...
			e.old = e.complete;
			e.complete = function() {
				e.queue !== false && c(this).dequeue();c.isFunction(e.old) && e.old.call(this)
			};return e
		},
		easing : {
	...
	c.offset = {
		initialize : function() {
			...
			this.doesNotIncludeMarginInBodyOffset = a.offsetTop !== h;a.removeChild(b);
		...
		position : function() {
			...
			e.left += parseFloat(c.css(b[0], "borderLeftWidth")) || 0;return {
				top : d.top - e.top,
				left : d.left - e.left
			}
		},
		...
		c.fn[d] = function(e) {
			...
			if (e !== B) return this.each(function() {
					if (h = fa(this)) h.scrollTo(!a ? e : c(h).scrollLeft(), a ? e : c(h).scrollTop());else
						this[d] = e
